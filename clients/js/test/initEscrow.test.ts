...
import { EscrowV1, fetchEscrowV1, initEscrowV1, MPL_HYBRID_PROGRAM_ID, Path } from '../src';
...
test('it can initialize the escrow', async (t) => {
    ...
    await initEscrowV1(umi, {
        ...
        solFeeAmount: 5,
        feeAta: findAssociatedTokenPda(umi, { mint: tokenMint.publicKey, owner: publicKey(feeLocation.publicKey) }),
...
test('it cannot use an invalid token mint', async (t) => {
    ...
    await t.throwsAsync(result, { message: /AnchorError caused by account: token. Error Code: AccountNotInitialized./ });
});
